...
assert dids, "no DIDS are defined, cannot continue with the algorithm"
...
# %% Check all paths
assert path_input_file.exists(), "Can't find required mounted path: {}".format(
    path_input_file
)
assert (
    path_input_file.is_file() | path_input_file.is_symlink()
), "{} must be a file.".format(path_input_file)
...
print(
    f"Selected input file: {path_input_file} {os.path.getsize(path_input_file)/1000/1000} MB"
)
