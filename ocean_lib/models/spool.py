...
class SPool(BToken):    
    ...
    #============================================================
    #to support easier Ocean flows
    def addLiquidity(self, num_DT_base:int, num_OCEAN_base:int,
                     from_wallet: Wallet):
        self._addLiquidity(self._DT_address, num_DT_base,
                           constants.INIT_POOL_WEIGHT_DT, from_wallet)
        self._addLiquidity(self._OCEAN_address, num_OCEAN_base,
                           constants.INIT_POOL_WEIGHT_OCEAN, from_wallet)

    def _addLiquidity(self, token_address: str, num_add_base: int,
                      weight_base: int, from_wallet: Wallet):
        assert num_add >= 0
        if num_add == 0: return
        
        num_before_base = self.getBalance_base(token_address)
        num_add_base = DTtoBase(num_add)
        ...
        assert token.balanceOf_base(from_wallet.address) >= num_add_base, \
            "insufficient funds"
        token.approve(self.getAddress(), num_add_base, from_wallet)
        self.rebind(token_address, num_after_base, weight_base, from_wallet)

    def removeLiquidity(self, num_DT_base:int, num_OCEAN_base:int,
                        from_wallet: Wallet):
        self._removeLiquidity(self._DT_address, num_DT_base,
                              constants.INIT_POOL_WEIGHT_DT, from_wallet)
        self._removeLiquidity(self._OCEAN_address, num_OCEAN_base,
                              constants.INIT_POOL_WEIGHT_OCEAN, from_wallet)

    def _removeLiquidity(self, token_address:str, num_remove_base: int,
                         weight_base:int, from_wallet: Wallet):
        assert num_remove >= 0
        if num_remove == 0: return
        
        num_before_base = self.getBalance_base(token_address)
        num_remove_base = DTtoBase(num_remove)
        num_after_base = num_before_base - num_remove_base
        
        self.rebind(token_address, num_after_base, weight, from_wallet)

    def buyDataTokens(self, num_DT_base:int, max_num_OCEAN_base:int,
                      from_wallet: Wallet):
        """
        ...
        OCEAN = BToken(self._web3, self._OCEAN_address)
        OCEAN.approve(pool_address, max_num_OCEAN_base, from_wallet)
        
        self.swapExactAmountOut(
            tokenIn_address = self._OCEAN_address,         #entering pool
            maxAmountIn_base = max_num_OCEAN_base,         #""
            tokenOut_address = self._DT_address,           #leaving pool
            tokenAmountOut = num_DT_base,                  #""
            maxPrice = 2 ** 255, #here we limit by max_num_OCEAN, not price
            from_wallet = from_wallet,
        )

    def sellDataTokens(self, num_DT_base:int, min_num_OCEAN_base:int,
                       from_wallet: Wallet):
        """
        ...
        """
        pool_address = self.address
        DT = BToken(self._web3, self._DT_address)
        DT.approve(pool_address, num_DT_base, from_wallet=from_wallet)
        
        self.swapExactAmountIn(
            tokenIn_address = self._DT_address,          #entering pool
            tokenAmountIn_base = num_DT_base,            #""
            tokenOut_address = self._OCEAN_address,      #leaving pool
            minAmountOut_base = min_num_OCEAN_base,      # ""
            maxPrice = 2 ** 255, #here we limit by max_num_OCEAN, not price
            from_wallet = from_wallet,
        )

    def getDTSpotPrice_base(self) -> int:
        return self.getSpotPrice(
            tokenIn_address = self._OCEAN_address,
            tokenOut_address = self._DT_address)

    def addLiquidity_Finalized(
            self, num_BPT_base:int, max_num_DT_base:int,
        ...
        pool.joinPool(num_BPT_base, [num_DT_base, num_OCEAN_base],
                      from_wallet=from_wallet)
