...
from ocean_lib.models.v4.fixed_rate_exchange import (
    FixedRateExchangeV4,
    FixedRateExchangeV4Details,
    FixedRateExchangeV4FeesInfo,
    FixedExchangeBaseInOutData,
)
from ocean_lib.models.v4.models_structures import FixedData
...
def test_properties(web3, config):
    ...
    fixed_exchange = FixedRateExchangeV4(
        web3, get_address_of_type(config, "FixedPrice")
    )

    assert (
        fixed_exchange.event_ExchangeCreated.abi["name"]
        == FixedRateExchangeV4.EVENT_EXCHANGE_CREATED
    )
    assert (
        fixed_exchange.event_ExchangeRateChanged.abi["name"]
        == FixedRateExchangeV4.EVENT_EXCHANGE_RATE_CHANGED
    )
    assert (
        fixed_exchange.event_ExchangeActivated.abi["name"]
        == FixedRateExchangeV4.EVENT_EXCHANGE_ACTIVATED
    )
    assert (
        fixed_exchange.event_ExchangeDeactivated.abi["name"]
        == FixedRateExchangeV4.EVENT_EXCHANGE_DEACTIVATED
    )
    assert fixed_exchange.event_Swapped.abi["name"] == FixedRateExchangeV4.EVENT_SWAPPED
    assert (
        fixed_exchange.event_TokenCollected.abi["name"]
        == FixedRateExchangeV4.EVENT_TOKEN_COLLECTED
    )
    assert (
        fixed_exchange.event_OceanFeeCollected.abi["name"]
        == FixedRateExchangeV4.EVENT_OCEAN_FEE_COLLECTED
    )
    assert (
        fixed_exchange.event_MarketFeeCollected.abi["name"]
        == FixedRateExchangeV4.EVENT_MARKET_FEE_COLLECTED
    )
...
def test_exchange_rate_creation(
    ...
    assert (exchange_details[FixedRateExchangeV4Details.DT_SUPPLY]) == 0
    assert (exchange_details[FixedRateExchangeV4Details.BT_SUPPLY]) == 0
    ...
    assert fee_info[FixedRateExchangeV4FeesInfo.MARKET_FEE] == market_fee
    assert (
        fee_info[FixedRateExchangeV4FeesInfo.MARKET_FEE_COLLECTOR]
        == another_consumer_wallet.address
    )
    assert fee_info[FixedRateExchangeV4FeesInfo.OPF_FEE] == 0
    assert fee_info[FixedRateExchangeV4FeesInfo.MARKET_FEE_AVAILABLE] == 0
    assert fee_info[FixedRateExchangeV4FeesInfo.OCEAN_FEE_AVAILABLE] == 0
    ...
    event_log = fixed_exchange.get_event_log(
        event_name=FixedRateExchangeV4.EVENT_SWAPPED,
        from_block=tx_receipt.blockNumber,
        to_block=web3.eth.block_number,
        filters=None,
    )
    ...
    assert (exchange_details[FixedRateExchangeV4Details.DT_SUPPLY]) == amount_dt_to_sell
    assert (exchange_details[FixedRateExchangeV4Details.BT_SUPPLY]) == 0
    ...
    logs = fixed_exchange.get_event_log(
        event_name=FixedRateExchangeV4.EVENT_TOKEN_COLLECTED,
        from_block=tx_receipt.blockNumber,
        to_block=web3.eth.block_number,
        filters=None,
    )
    ...
    assert (
        ...
        == bt_balance_before
        + fee_info[FixedRateExchangeV4FeesInfo.MARKET_FEE_AVAILABLE]
    )
