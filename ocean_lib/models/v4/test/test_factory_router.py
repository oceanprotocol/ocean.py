...
def factory_router(web3,config):
    _factory_router = FactoryRouter(
        web3,
        get_address_of_type(config,"Router"),
    )
    return _factory_router
...
def test_update_opf_fee(web3, config,factory_router):
...
def test_fail_update_opf_fee(web3, config,factory_router):
...
def test_fail_add_factory_owner(web3, config, factory_router):
    ...
    assert factory_router.factory() == get_address_of_type(config,ERC721FactoryContract.CONTRACT_NAME)
    assert (
        err.value.args[0]
...
def test_fail_add_factory_not_owner(web3, config, factory_router):
...
def test_fixed_rate(web3, config, factory_router):
...
def test_fail_add_fixed_rate_contract(web3, config, factory_router):
...
def test_fail_add_pool_template(web3, config, factory_router):
...
def test_add_pool_template(web3, config, factory_router):
...
def test_buy_dt_batch(web3, config, factory_router):
    ...
    nft_factory = ERC721FactoryContract(
        web3=web3, address=get_address_of_type(config,ERC721FactoryContract.CONTRACT_NAME)
    )
    ...
    ocean_contract.approve(
        get_address_of_type(config,ERC721FactoryContract.CONTRACT_NAME), 2 ** 256 - 1, factory_deployer
    )
    ocean_contract.approve(
