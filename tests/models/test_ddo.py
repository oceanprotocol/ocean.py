...
def test_ddo_on_chain():
    ...
    ddo_address = get_contracts_addresses('ganache', config)[MetadataContract.CONTRACT_NAME]
    dtfactory_address = get_contracts_addresses('ganache', config)[DTFactory.CONTRACT_NAME]
    ...
    try:
        ...
        logs = ddo_registry.event_MetadataUpdated().processReceipt(ddo_registry.get_tx_receipt(txid))
        assert not logs, f'should be no logs for MetadataUpdated, but seems there are some logs: tx {txid}, logs {logs}'
    except ValueError:
