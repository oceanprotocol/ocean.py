...
def test_fixed_rate_exchange(alice_ocean, alice_wallet, T1, bob_wallet, T2, contracts_addresses):
    """
    tests:
        create
        generateExchangeId
    ...
    assert num_ex == len(fixed_ex.getExchanges()), f'num exchanges do not match num of exchange ids.'
    ...
    assert ocn_token.get_tx_receipt(ocn_token.approve(fixed_ex.address, 1, bob_wallet)).status == 1, f'approve failed'
    ...
    t2_ex_id = fixed_ex.generateExchangeId(ocean_t, T2.address, alice_wallet.address).hex()
    ...
    assert fixed_ex.get_tx_receipt(fixed_ex.deactivate(t2_ex_id, alice_wallet)).status == 1, f'exchange {t2_ex_id} deactivate failed.'
    assert fixed_ex.isActive(t2_ex_id) is False, f'exchange {t2_ex_id} is active, but it should be deactivated.'
    ...
    assert run_failing_tx(
        fixed_ex, fixed_ex.buy_data_token, t2_ex_id, amount, bob_wallet
            ...
            f'maybe the FixedRateExchange is already approved as spender for bob_wallet.'
    assert ocn_token.allowance(bob_wallet.address, fixed_ex.address) == base_token_quote, f''
    assert fixed_ex.get_tx_receipt(fixed_ex.activate(t2_ex_id, alice_wallet)).status == 1, f'exchange {t2_ex_id} deactivate failed.'
    assert fixed_ex.isActive(t2_ex_id) is True, f'exchange {t2_ex_id} is not active, but it should be.'
    ...
    assert fixed_ex.getRate(t2_ex_id) == rate2, f'T2 exchange rate does not match {rate2}, got {fixed_ex.getRate(t2_ex_id)}'
    assert fixed_ex.getRate(ex_id) == rate, f'T1 exchange rate does not match {rate}, got {fixed_ex.getRate(ex_id)}'
    rate2 = to_base_18(0.75)
    assert fixed_ex.get_tx_receipt(fixed_ex.setRate(t2_ex_id, rate2, alice_wallet)).status == 1, f'setRate failed.'
    assert fixed_ex.getRate(t2_ex_id) == rate2, f'T2 exchange rate does not match {rate2}, got {fixed_ex.getRate(t2_ex_id)}'

    assert run_failing_tx(fixed_ex, fixed_ex.setRate, t2_ex_id, to_base_18(0.0), alice_wallet) == 0, f'should not accept rate of Zero.'
    assert run_failing_tx(fixed_ex, fixed_ex.setRate, t2_ex_id, -to_base_18(0.05), alice_wallet) == 0, f'should not accept a negative rate.'
    assert fixed_ex.get_tx_receipt(fixed_ex.setRate(t2_ex_id, to_base_18(1000.0), alice_wallet)).status == 1, f'setRate failed.'
