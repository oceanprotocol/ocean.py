...
def test_setSwapFee_fails(network,
    ...
    with pytest.raises(Exception):
        pool.setSwapFee(to_base_18(0.011), from_wallet=bob_wallet) #not ok, bob isn't controller
...
def test_2tokens_basic(network, T1, T2,
    ...
    bal2 = from_base_18(T2.balanceOf(alice_address)) >= 10.0
...
def test_public_pool(network, bob_wallet):
    ...
    pool.joinPool(poolAmountOut_base=to_base_18(10.0),  #10 BPT
...
def test_rebind_more_tokens(network, T1, T2, alice_wallet):
    ...
    with pytest.raises(Exception): 
        pool.rebind(T1.address, to_base_18(120.0), to_base_18(9.0),
                    from_wallet=alice_wallet)
        
    #sufficient allowance
    T1.approve(pool.address, to_base_18(30.0),
               from_wallet=alice_wallet)
    pool.rebind(T1.address, to_base_18(120.0), to_base_18(9.0),
                from_wallet=alice_wallet)
...
def test_gulp(network, T1, alice_wallet):
    ...
    pool.bind(T1.address, to_base_18(2.0), to_base_18(50.0),
              from_wallet=alice_wallet)
